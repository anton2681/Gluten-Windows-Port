@echo off
rem Sanity check: load Gluten plugin but DISABLE offload (spark.gluten.enabled=false).
rem Useful to verify the JAR / DLL loading path is fine when investigating native crashes.
setlocal
