...
| Joins with NULL keys in key column | NULL keys leave velox memory state in a way that produces bogus reservations (e.g. 850 GiB request); periodic stats poll previously deref'd `0xff...` inside `protobuf::SerializeToArray` and crashed the JVM | JVM crash fixed in `cpp/core/jni/JniWrapper.cc` (SEH guard around `ByteSizeLong` / `SerializeToArray` — stats degrade to empty blob instead of killing the process). Earlier `Negative buffer resize: -92` symptom fixed in `cpp/velox/shuffle/VeloxHashShuffleWriter.cc::valueBufferSizeForBinaryArray` (uint64 underflow + div-by-zero when `totalInputNumRows_==0`, undefined behavior on MSVC). Underlying NULL-key alloc loop in HashJoin still fails the LEFT JOIN query with OOM — use `WHERE key IS NOT NULL` to dodge | Root-cause the velox NULL-key allocation loop in HashJoinNode build (or columnar batch serialize); alternative is to detect `joinHasNullKeys` in `SubstraitToVeloxPlan` SHJ path and `VELOX_NYI` so gluten falls back to vanilla Spark |
