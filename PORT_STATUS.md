...
| Joins with NULL keys in key column | NULL-key join paths produce a truncated columnar shuffle stream. Writer emits a `BlockPayload` header that says `numBuffers=N` but the bytes following only contain K<N buffers worth of data; reader hits EOF mid-stream and (without the guard below) reads uninit stack bytes as the next bufferLength → bogus 200+ GiB reservation requests / `Negative buffer resize: -92` / `LZ4 frame error` symptoms. Earlier JVM crash from periodic stats poll → `protobuf::SerializeToArray` AV. | (1) JVM crash fixed via SEH guard in `cpp/core/jni/JniWrapper.cc`. (2) Bogus header values can no longer overflow Arrow `Resize` — `cpp/core/shuffle/Payload.cc` now validates each buffer length on read and on write (16 GiB sanity bound, short-read detection, init-before-Read, per-buffer index in error). (3) Two MSVC-fragile `(isNull - 1) & stringView.size()` branchless masks in `cpp/velox/shuffle/VeloxHashShuffleWriter.cc` replaced with explicit `if`/`else`. (4) `valueBufferSizeForBinaryArray` div-by-zero / underflow guarded when `totalInputNumRows_==0`. **Still failing**: T2 LEFT JOIN with NULL keys, now with the clean message `Failed reading buffer 5/7 (type=uncompressed, numRows=1): Short read got 0 bytes`. Workaround for users: `WHERE key IS NOT NULL` before the join. | Find the writer-side path that produces a truncated stream for a single-row NULL-key payload. Either some `outputStream->Write(buffer)` call doesn't write the full buffer (silent partial write) or `numBuffers_` is set higher than the actual entries in `buffers_` along a NULL-key-specific code path that the new write-time assertion didn't catch. Suggest instrumenting `BlockPayload::serialize` with byte counters to verify against expected payload size. |
